...
//		public bool DontPaintOverBlack = true; // so that outlines are reserved when painting
		...
		[Space(10)] // custom brushes, patterns, lines
...
//		private int customBrushHeightHalf;
		...
		Transform previewLineCircleStart; // clone for start circle
	...
	//	*** private variables, no need to touch ***
		...
		void Awake()
		{
			...
			if (go==null) 
			{
				Debug.LogWarning("GameObject EventSystem is missing from scene, will have problems with UI",gameObject); 
			}else{
		...
		void StartupValidation()
		{
			...
			{
				Debug.LogError("userInterface not assigned", gameObject);
			}
...
//				Debug.LogWarning("No linerenderer component added. LineDrawing mode preview requires it",gameObject);
		...
		public void InitializeEverything() 
		{
			...
			if (myRenderer.material.GetTexture(targetTexture)==null)
			{
				...
				// we keep current maintex and read it as "clear pixels array" (so when "new image" is clicked, original texture is restored
				ReadClearingImage();	
				myRenderer.material.SetTexture(targetTexture, drawingTexture);
			}
...
//			if (useMaskImage) ReadMaskImage();
			
			// undo system, NOTE: only 1 step
			if (undoEnabled)
			{
		...
		void MousePaint ()
		{
			...
			if (Input.GetKeyDown("u") && undoEnabled) DoUndo();
...
//			if (connectBrushStokes && Vector2.Distance(pixelUV,pixelUVOld)>brushSize)
		...
		public void DrawShapeLinePreview(int x,int y)
		{
			...
			if (!haveStartedLine)
			{
				...
				previewLineCircle.position = worldPixelPos;
			}
		...
		void DrawLineWithBrush(Vector2 start, Vector2 end)
		{
			...
			for (;;)
			{
				if (hiQualityBrush)
				{
					...
					pixelCount++;
					if (pixelCount>brushSizeDiv4) // bit slower if these are commented
					{
						pixelCount=0;
						DrawCustomBrush(x0,y0);
					}
		...
		*/
		...
		public void DoUndo()
		{
			if (undoEnabled)
			{
				...
				drawingTexture.LoadRawTextureData(undoPixels);
				drawingTexture.Apply(false);
			}
		}
		...
		public void ReadClearingImage()
		{
			...
			{
				c = tempPixels[i];
				clearPixels[pixel] = c.r;
				clearPixels[pixel+1] = c.g;
				clearPixels[pixel+2] = c.b;
				clearPixels[pixel+3] = c.a;
				pixel += 4;
			}
		}
...
//			return transform.TransformPoint(new Vector3(localX,localY, 0));
		...
		void UpdateLineModePreviewObjects()
		{
			...
			if (previewLineCircle) 
			{
				previewLineCircle.GetComponent<SpriteRenderer>().color = paintColor;
				previewLineCircle.transform.localScale = Vector3.one*brushSize*0.4f;
			}

			if (previewLineCircleStart) 
			{
				previewLineCircleStart.GetComponent<SpriteRenderer>().color = paintColor;
				previewLineCircleStart.transform.localScale = Vector3.one*brushSize*0.4f;
			}
		}
