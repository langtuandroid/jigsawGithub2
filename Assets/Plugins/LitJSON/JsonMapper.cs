...
namespace LitJson
{
    ...
    public class JsonMapper
    {
        ...
        private static void AddObjectMetadata (Type type)
        {
			...
			{
				object[] serializedAttributeList = f_info.GetCustomAttributes(typeof(JsonSerialize),true);
				
				if(f_info.IsPublic || serializedAttributeList.Length > 0)
				{
	                PropertyMetadata p_data = new PropertyMetadata ();
	                p_data.Info = f_info;
	                p_data.IsField = true;
        ...
        private static void AddTypeProperties (Type type)
        {
            ...
            foreach (PropertyInfo p_info in type.GetProperties ()) 
			{
				...
				object[] nonSerializedAttributeList = p_info.GetCustomAttributes(typeof(JsonIgnore),true);
//				object[] serializedAttributeList = p_info.GetCustomAttributes(typeof(JsonSerialize),true);
				
				if(nonSerializedAttributeList.Length > 0)
					continue;

//				if((p_info.GetGetMethod() != null && p_info.GetSetMethod() != null) || serializedAttributeList.Length > 0)
//				{
				PropertyMetadata p_data = new PropertyMetadata ();
                p_data.Info = p_info;
...
//				}
			...
			foreach (FieldInfo f_info in type.GetFields (BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic)) 
			{
				object[] nonSerializedAttributeList = f_info.GetCustomAttributes(typeof(JsonIgnore),true);
				object[] serializedAttributeList = f_info.GetCustomAttributes(typeof(JsonSerialize),true);
		...
		private static void ReadPastCurrentObject (JsonReader reader)
		{	
			do
			{
				reader.Read();
				...
				else if (reader.Token == JsonToken.ObjectEnd || reader.Token == JsonToken.ArrayEnd)
				{
					return;
				} 
			}
        ...
        private static void RegisterBaseExporters ()
        {
            ...
            base_exporters_table[typeof (DateTime)] =
                delegate (object obj, JsonWriter writer) {
                    writer.Write (((DateTime) obj).ToString("R"));
                };
